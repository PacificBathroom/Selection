...
const norm = (s) => (s || '').toLowerCase().replace(/[\s_]+/g, ' ').replace(/[^a-z0-9\- ]+/g, '').trim();
function scoreCandidate(title, url, q) {
  const t = norm(title);
  ...
  try {
    ...
    if (idxResp.ok) {
      ...
      $('sitemap > loc').each((_, el) => {
        ...
        if (/\/product(-\d+)?-sitemap.*\.xml$/i.test(loc) || /\/product-sitemap.*\.xml$/i.test(loc)) list.push(loc);
      });
...
async function enrichTopMatches(urls, q, cap = 12) {
  ...
  const detailed = await Promise.all(top.map(async it => {
    try {
