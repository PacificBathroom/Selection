...
function cleanText(s?: string | null, maxLen = 900): string | undefined {
...
async function drawCover(pptx: any, client: ClientInfo, sections: Section[]) {
  ...
  s.addText(
    `${sections.length} section${sections.length === 1 ? '' : 's'} • ${totalProducts} product${
      totalProducts === 1 ? '' : 's'
    }`,
    {
      x: M,
...
export async function exportDeckPptx({
  ...
  // Safe dynamic import that works with/without esModuleInterop
  ...
  for (const sec of sections) {
    ...
    for (const p of prods) {
      ...
      const desc = cleanText((p as any).description);
      if (desc) {
        slide.addText(desc, {
          x: RX,
          y,
      ...
      if ((p as any).features?.length) {
        ...
        y += 0.35;
        const items = ((p as any).features as string[]).slice(0, 8).map((f) => `• ${f}`);
        slide.addText(items, {
          x: RX,
          y,
          w: SLIDE_W - RX - M,
      ...
      const specs = normalizeSpecs((p as any).specs);
      if (specs.length) {
        ...
        y += 0.35;
        slide.addText((p as any).specPdfUrl, {
          x: RX,
          y,
          w: SLIDE_W - RX - M,
          h: 0.3,
