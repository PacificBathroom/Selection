// 1) Keep your imports & types the same…
...
export default function ProductDrawer({ product, onClose }: Props) {
  ...
  // ✅ 2) Non-null local after the guard
  const p: Product = product;

  // ✅ 3) Use `p` inside exportPDF and JSX
      ...
      <div className="absolute right-0 top-0 h-full w-full max-w-2xl bg-white shadow-xl overflow-y-auto">
        <div className="p-6 border-b flex items-start justify-between">
          ...
          <div className="flex items-center gap-2">
            <button onClick={exportPDF} className="rounded-lg bg-brand-600 text-white px-3 py-2 text-sm shadow hover:opacity-90">
              Export PDF
            </button>
            <button onClick={onClose} className="rounded-lg border px-3 py-2 text-sm hover:bg-slate-50">
              Close
            </button>
          </div>
        </div>
            ...
            <div className="space-y-6">
              ...
              {(p.compliance ?? []).length > 0 && (
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {(p.compliance ?? []).map((c: string, i: number) => <Tag key={i}>{c}</Tag>)}
                  </div>
                </div>
              )}
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {(p.assets ?? []).map((a: { label: string; url: string }, i: number) => (
                      <a key={i} href={a.url} target="_blank" rel="noreferrer" className="rounded-lg border px-3 py-2 text-sm hover:bg-slate-50">
                        {a.label}
                      </a>
                    ))}
                  </div>
              ...
              {p.sourceUrl && (
                <p className="text-xs text-slate-500">
                  Imported from <a className="underline" href={p.sourceUrl} target="_blank" rel="noreferrer">{p.sourceUrl}</a>
                </p>
              )}
            </div>
          </div>
