...
async function pdfFirstPageToPng(pdfUrl?: string): Promise<string | undefined> {
  try {
    ...
    const worker: any = await import("pdfjs-dist/build/pdf.worker.min.mjs");
    pdfjs.GlobalWorkerOptions.workerSrc = worker;

    const doc = await pdfjs.getDocument({ url: pdfUrl, useSystemFonts: true }).promise;
  ...
  for (const p of products) {
    ...
    if (imgData) {
      ...
      s.addShape(pptx.ShapeType.roundRect, {
        ...
        line: { color: "DDDDDD" }, // no boolean here
      } as any);
      s.addText("Image", {
...
// alias to satisfy existing imports like `exportPptx(...)`
