...
// dataURL -> verified PNG dataURL (so PPTX embeds happily)
...
async function fetchAsDataUrl(rawUrl?: string): Promise<{ ok: boolean; dataUrl?: string; msg?: string }> {
  ...
  if (!ct.startsWith("image/")) ct = "image/jpeg"; // be lenient
  const dataUrl = `data:${ct};base64,${b64}`;

  const check = await ensurePngDataUrl(dataUrl);
  if (!check.ok || !check.pngUrl) return { ok: false, msg: check.msg || "invalid image" };
  return { ok: true, dataUrl: check.pngUrl };
}

// PptxGenJS needs 'image/<type>;base64,...' (no 'data:')
...
export async function exportSelectionToPptx(rows: Product[], client: ClientInfo) {
  ...
  // Slide size constants (guard if library varies slightly)
  const slideW = 10.0;
  const slideH = 5.625;
  ...
  // Layout calculated from slide height so nothing goes off page
  const barH = 0.30;
  const barY = slideH - barH;               // bottom bar sits on page
  const descH = 0.48;
  const descY = barY - 0.35;                // safe spacing above the bar

  const L = {
    title:   { x: 0.5, y: 0.5, w: 9.0, h: 0.7 },
    img:     { x: 0.5, y: 1.1, w: 5.2, h: 3.7 },       // left
  ...
  for (const row of rows as unknown as Record<string, any>[]) {
    ...
    // Visible stamp to prove this exporter ran
    ...
    if (im.ok && im.dataUrl) {
      try {
        ...
        s.addText("addImage failed", { x: L.img.x, y: L.img.y + L.img.h/2 - 0.2, w: L.img.w, h: 0.4, align: "center", fontSize: 12, color: "667085" } as any);
      }
    } else {
      s.addShape(PptxGenJS.ShapeType.roundRect, { ...L.img, fill: { color: brand.faint }, line: { color: "D0D7E2", width: 1 } } as any);
      s.addText(`Image unavailable (${im.msg || "unknown"})`, { x: L.img.x, y: L.img.y + L.img.h/2 - 0.2, w: L.img.w, h: 0.4, align: "center", fontSize: 12, color: "667085" } as any);
    }
    ...
    // Description (bottom, inside page)
    if (description) {
...
// Preferred export name (distinct from any old exporter)
export const exportPptxV2 = exportSelectionToPptx;
// Back-compat alias (if something still imports exportPptx)
